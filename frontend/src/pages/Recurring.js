...
export default function Recurring() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(r.data); setTxns(t.data);
  ...
  const cancelMerchant = async (merchant) => {
    if (!window.confirm(`Mark all "${merchant}" charges as cancelled? This deletes them from history.`)) return;
    const match = txns.filter(t => t.merchant.trim().toLowerCase() === merchant.trim().toLowerCase());
    try {
      for (const t of match) await api.delete(`/transactions/${t.id}`);
      toast.success(`Removed ${match.length} ${merchant} charges`);
      load();
    } catch (e) { toast.error(formatApiError(e)); }
  };

  const total = items.reduce((s, r) => s + r.monthly_cost, 0);
  const annual = items.reduce((s, r) => s + r.annual_cost, 0);
  ...
  return (
    <Page>
      ...
      {loading ? (
        <>
          <div className="grid md:grid-cols-3 gap-4 mb-5">{Array.from({ length: 3 }).map((_, i) => <Skel key={i} className="h-20" />)}</div>
          <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skel key={i} className="h-14" />)}</div>
        </>
      ) : items.length === 0 ? (
        <Card className="p-2">
          <EmptyState icon={Repeat2} title="Nothing recurring yet"
        ...
        <>
          ...
          <Card className="overflow-hidden" data-testid="recurring-list">
            <div className="divide-y divide-sand-200">
              {items.map((r, i) => (
                <div key={i} className="px-4 py-4 flex items-center gap-4" data-testid={`recurring-${i}`}>
                  <div className="w-10 h-10 rounded-full bg-sand-100 flex items-center justify-center text-sm font-medium">{r.merchant.slice(0,1).toUpperCase()}</div>
                  ...
                  <div className="text-right hidden sm:block">
                    <div className="text-xs text-[color:var(--text-secondary)] flex items-center gap-1 justify-end">
                      <CalendarClock size={11} /> {r.next_expected ? `next ~ ${shortDate(r.next_expected)}` : "monthly"}
                    </div>
                    <div className="text-xs text-[color:var(--text-muted)] mt-0.5">last {shortDate(r.last_date)}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-num tabular-nums font-medium">{HKD(r.monthly_cost)}<span className="text-[10px] text-[color:var(--text-secondary)]">/mo</span></div>
                    <div className="text-[11px] text-[color:var(--text-secondary)] font-num">{HKD(r.annual_cost)}<span className="text-[10px]">/yr</span></div>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => cancelMerchant(r.merchant)} data-testid={`recurring-cancel-${i}`} className="flex-shrink-0">
