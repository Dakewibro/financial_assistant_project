...
1. Start the backend with `STORAGE_MODE=memory` or a valid `MONGODB_URI`, then start the frontend with `VITE_API_BASE_URL` / `VITE_BACKEND_URL` pointing at the backend (or rely on the Vite dev proxy + local backend).
...
8. Load the `subscription-creep` scenario (`import.json` via `/api/import`) and confirm recurring items plus recurring-related insights appear.
9. Load the `transport-budget` scenario and confirm period-cap and category-share alerts appear.
10. Open **Dashboard** and **Insights** and confirm category totals and the personal/business split match the imported or entered data.
